...
int main(int argc, char* argv[])
{
	...
	if (!geometryContainer.readCirclesData("Circles.txt") ) {
		std::cout << "Could not read circles file " << "Circles.txt" << std::endl;
		return 0;
	}
	if (!geometryContainer.readEllipsesData("Ellipses.txt") ) {
		std::cout << "Could not read ellipses file " << "Ellipses.txt" << std::endl;
		return 0;
	}
	...
	if (!outfile.is_open()) {
		std::cout << "Could not open output file " << "out.txt" << std::endl;
		return 0;
	}
